...
export default function Product() {
  ...
  const [purchaseOrderCreated, setPurchaseOrderCreated] = useState(dayjs());
  const [purchaseOrderSent, setPurchaseOrderSent] = useState(dayjs());
  const [purchaseOrderQuantity, setPurchaseOrderQuantity] = useState(0)
  const [purchaseOrderTotalPrice, setPurchaseOrderTotalPrice] = useState(0)
  const [purchaseOrderDiscount, setPurchaseOrderDiscount] = useState(0)
  const [poTotalExcludeVat, setPoTotalExcludeVat] = useState(0)
  const [purchaseOrderVat, setPurchaseOrderVat] = useState(0)
  const [poTotalIncludeVat, setPoTotalIncludeVat] = useState(0)
  ...
  const fetchPurchaseOrders = async () => {
    try {
      const apiPo = await axios.get(`${url}/api/purchaseorder/`);
      const data = apiPo.data;
      setGetPurchaseOrder(data);
      // console.log(data);
    } catch (err) {
      console.log(err);
    }
  ...
  return (
    <>
      <div style={{ marginLeft: "300px", marginTop: "20px" }}>
        <span>
          You have {getPurchaseOrder.length}{" "}
          {getPurchaseOrder.length > 1 ? "purchase orders" : "purchase order"}{" "}
          in total
        </span>
        <Button
        ...
        <Dialog open={openAdd}>
          <DialogTitle>Add a New Purchase Order</DialogTitle>
          <DialogContent>
            <TextField
              sx={{ mt: 1 }}
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <Stack spacing={3}>
                <DatePicker
                  ...
                  onChange={(newValue) => {
                    setPurchaseOrderCreated(newValue);
                  }}
            ...
            <LocalizationProvider dateAdapter={AdapterDayjs}>
              <Stack spacing={3}>
                <DatePicker
                  ...
                  openTo="year"
                  views={["year", "month", "day"]}
                  value={purchaseOrderSent}
                  onChange={(newValue) => {
                    setPurchaseOrderSent(newValue);
                  }}
                  renderInput={(params) => <TextField {...params} />}
                />
              </Stack>
            ...
            <TextField
              ...
              label="Total Price"
              variant="standard"
              value={purchaseOrderTotalPrice}
              onChange={(e) => setPurchaseOrderTotalPrice(e.target.value)}
            />
            <TextField
              sx={{ mt: 1 }}
              label="Discount"
              ...
              onChange={(e) => setPurchaseOrderDiscount(e.target.value)}
            />
            <TextField
              sx={{ mt: 1 }}
              label="Total Price Exclude Vat"
              variant="standard"
              value={poTotalExcludeVat}
              onChange={(e) => setPoTotalExcludeVat(e.target.value)}
            />
            <TextField
              sx={{ mt: 1 }}
              label="Vat"
              variant="standard"
              value={purchaseOrderVat}
              onChange={(e) => setPurchaseOrderVat(e.target.value)}
            />
            <TextField
              sx={{ mt: 1 }}
              label="Total Include Vat"
              variant="standard"
              value={poTotalIncludeVat}
              onChange={(e) => setPoTotalIncludeVat(e.target.value)}
            />
          </DialogContent>
          <DialogActions>
